...
function(input, output, session) {
  ...
  # Load model and  data
  climate_data = readRDS("data/climate_data_province_v2.RDS")
  province_params = readRDS("data/province_sites_parameters.RDS")
  model_pred = readRDS("data/model_predictions_province_29Nov.RDS")
  start_year = 2023
  ...
  observe({
    ...
    if (input$temperature_type == "discrete") {
      ...
      updateSliderInput(session, "SMC_cov", value = 0)
  ...
  output$model_plot_monthly <- renderPlotly({
    ...
    if (input$output_type == "inc") {
      if(input$age_groups == "under 5 years") {
        model_out = "inc_5"
        plot_title = "Incidence per 100000 (<5yo)"
      } else {
        model_out = "inc_all"
        plot_title = "Incidence per 100000 (all ages)"
      }
